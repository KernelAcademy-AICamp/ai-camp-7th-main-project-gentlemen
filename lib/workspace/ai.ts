...
export function cardSystemPrompt(survey: SurveyProfile, format: CardFormat): string {
  ...
  const qualityRules = [
    ...
    "[예시 — 이 수준으로 쓴다 (도메인 무관·수준만 참고·복붙 금지)]\n" +
      "  (○ 좋음) head '복리는 이자에 이자가 붙는 것' / body '100만원을 연 10%에 넣으면 1년 뒤 110만원, 2년 뒤엔 121만원. 늘어난 10만원에도 이자가 붙어요.' ← 정의 + 구체 숫자\n" +
      "  (✗ 나쁨) head '복리의 핵심' / body '복리는 정말 중요해요. 꼭 기억하세요.' ← 알맹이 없는 빈 표현",
  ...
  return [
    ...
    ...common,
    ...qualityRules,
    "[글자수 상한 — 초과 금지] headline ≤24자 / body ≤48자(줄바꿈 \\n 허용). 이 상한은 카드 레이아웃이 깨지지 않는 물리적 한계다 — 내용이 많으면 다음 장으로 나누고, 한 장에 욱여넣지 않는다.",
