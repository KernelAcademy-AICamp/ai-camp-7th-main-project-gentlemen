...
> **적용 메모 (Kup):** 이 문서는 Kup 워크스페이스 UI의 디자인 SoT다. 리스킨은 `app/globals.css`의
> `@theme` 토큰 **값만** 교체해 전체에 반영한다(팀이 설계해둔 교체점). Airbnb Cereal은 상용 폰트라
> **Inter**로 대체하고 한글은 Pretendard로 폴백한다. `components/workspace/CardCanvas.tsx`의 카드 테마
> 프리셋은 제품 콘텐츠라 이 팔레트로 덮지 않는다.
...
If Airbnb Cereal VF and Circular are unavailable, **Inter** is the closest open-source substitute. Adjust display headlines down by ~2% in line-height to match Cereal's slightly tighter cap height; otherwise the proportions transfer cleanly. (Kup uses Inter for Latin + Pretendard for Korean.)
